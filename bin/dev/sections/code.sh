...
#   - org.iris.order.*   (V8/V9 migrations, 2026-04-26)
#   - org.iris.product.* (V7 migration, 2026-04-26)
#   - org.iris.ml.*      (Phase B Customer Churn ONNX inference,
...
section_feature_slices() {
  ...
  for slice in order product ml; do
    local main_root="$SVC_DIR/src/main/java/org/iris/$slice"
    local test_root="$SVC_DIR/src/test/java/org/iris/$slice"
    if [ ! -d "$main_root" ]; then
