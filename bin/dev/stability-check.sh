...
# Try the new flat layout first (~/dev/iris/iris-ui, post-2026-04-26 migration),
# fall back to the old workspace-modern layout for backward compatibility.
UI_DIR="$(cd "$SVC_DIR/../iris-ui" 2>/dev/null && pwd || echo "")"
if [[ -z "$UI_DIR" || ! -d "$UI_DIR" ]]; then
  UI_DIR="$(cd "$SVC_DIR/../../js/iris-ui" 2>/dev/null && pwd || echo "")"
fi
if [[ -z "$UI_DIR" || ! -d "$UI_DIR" ]]; then
  echo "✗ UI repo not found — tried both:"
  echo "    $SVC_DIR/../iris-ui            (new flat layout)"
  echo "    $SVC_DIR/../../js/iris-ui      (old workspace-modern layout)"
