 ...
 *
 * <p>The model file path is configurable via the {@code iris.churn.model-path}
 ...
 * {@code iris_service.ml.train_churn} writes to by default).
 *
...
public class ChurnPredictor {
    ...
    public ChurnPredictor(
            @Value("${iris.churn.model-path:" + DEFAULT_MODEL_PATH + "}") String modelPath,
            @Value("${iris.churn.model-version:unspecified}") String modelVersionLabel
    ) {
    ...
    void loadModel() {
        if (!Files.exists(modelPath)) {
            log.warn("churn model NOT loaded — file missing at {} ; predictions will return 503 until the model is provisioned via the iris-churn-model ConfigMap (shared ADR-0062)",
                    modelPath);
            return;
        }
        try {
    ...
    public double predictProbability(float[] features) throws OrtException {
        if (!isReady()) {
            throw new IllegalStateException(
                    "Churn model not loaded — provision the .onnx via the iris-churn-model ConfigMap "
