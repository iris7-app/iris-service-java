...
```
bin/demo-up.sh     # terraform apply + bootstrap Argo CD + ESO  (~8 min)
bin/demo-down.sh   # terraform destroy                          (~5 min)
```
...
`bin/demo-up.sh` re-wires the Workload Identity annotation on every
...
| **Dashboard-as-code** | grizzly/jsonnet in git | Dashboards applied on every `demo-up`. | **No change**. This is already git-tracked and applied via `bin/demo-up.sh` extension. |
...
- `bin/demo-up.sh` / `bin/demo-down.sh` — the two-verb lifecycle.
