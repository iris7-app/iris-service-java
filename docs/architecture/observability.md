...
5. Click any span → jump to its Loki logs (trace ID propagated in MDC via
   `org.iris.observability.RequestIdFilter`).
...
`HealthIndicator` (`org.iris.observability.DatabaseReachabilityHealthIndicator`)
...
`org.iris.api.ApiExceptionHandler`, so the frontend can match the
...
`org.iris.messaging.KafkaConfig` — auto-create is disabled on the local
...
The filter (`org.iris.resilience.RateLimitingFilter`) validates the
